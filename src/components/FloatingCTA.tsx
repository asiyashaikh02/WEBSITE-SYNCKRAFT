...
export const FloatingCTA: React.FC = () => {
  ...
  if (isMobile) {
    return (
      <div className="fixed bottom-0 left-0 right-0 z-[100] bg-white/80 dark:bg-[#0A0A0B]/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/10 p-4 flex justify-around items-center shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
        <Link to="/book-demo" className="flex flex-col items-center gap-1 text-slate-600 dark:text-slate-400 text-[10px] font-bold uppercase hover:text-blue-600 transition-colors">
          <Calendar size={20} />
          Book Demo
        </Link>
        ...
        <Link to="/free-audit" className="flex flex-col items-center gap-1 text-slate-600 dark:text-slate-400 text-[10px] font-bold uppercase hover:text-emerald-600 transition-colors">
          <FileText size={20} />
          Free Audit
        </Link>
      </div>
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col gap-4">
      <Link to="/book-demo" className="bg-blue-600 hover:bg-blue-500 text-white shadow-xl shadow-blue-600/20 px-6 py-3 rounded-2xl font-bold flex items-center gap-2 transition-transform hover:-translate-y-1">
        <Calendar size={20} /> Book Demo
      </Link>
      <Link to="/free-audit" className="bg-emerald-600 hover:bg-emerald-500 text-white shadow-xl shadow-emerald-600/20 px-6 py-3 rounded-2xl font-bold flex items-center gap-2 transition-transform hover:-translate-y-1">
