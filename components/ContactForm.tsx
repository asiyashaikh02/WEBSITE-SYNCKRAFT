...
export const ContactForm: React.FC<ContactFormProps> = ({ theme }) => {
  const [status, setStatus] = useState<'idle' | 'submitting' | 'success'>('idle');
  const [consent, setConsent] = useState(false); // Added for Meta/Legal compliance
          ...
          <div className="reveal" style={{ transitionDelay: '0.2s' }}>
            <form onSubmit={handleSubmit} className={`p-6 sm:p-10 rounded-[3rem] border shadow-2xl ${
              ...
              <div className="grid sm:grid-cols-2 gap-6 mb-8">
                <div className="space-y-2">
                  <label htmlFor="name" className={`text-xs font-bold uppercase tracking-widest ml-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>Full Name</label>
                  <input required type="text" id="name" placeholder="Enter your name" className={`w-full px-6 py-4 rounded-2xl outline-none transition-all border ${theme === 'dark' ? 'bg-white/5 border-white/10 text-white focus:border-blue-500 focus:bg-white/10' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-600 focus:bg-white'}`} />
                </div>
                <div className="space-y-2">
                  <label htmlFor="email" className={`text-xs font-bold uppercase tracking-widest ml-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>Email Address</label>
                  <input required type="email" id="email" placeholder="name@company.com" className={`w-full px-6 py-4 rounded-2xl outline-none transition-all border ${theme === 'dark' ? 'bg-white/5 border-white/10 text-white focus:border-blue-500 focus:bg-white/10' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-600 focus:bg-white'}`} />
                </div>
              </div>

              <div className="space-y-2 mb-8">
                <label htmlFor="subject" className={`text-xs font-bold uppercase tracking-widest ml-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>Subject</label>
                <select id="subject" className={`w-full px-6 py-4 rounded-2xl outline-none transition-all border appearance-none ${theme === 'dark' ? 'bg-white/5 border-white/10 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-600'}`}>
                  <option value="venture">Venture Proposal</option>
                  <option value="partnership">Strategic Partnership</option>
                  <option value="investment">Investment Inquiry</option>
                  <option value="other">Other Query</option>
              ...
              {/* MANDATORY CONSENT BOX FOR META VERIFICATION */}
              <div className="flex items-start gap-3 mb-10 ml-1">
                ...
                <label htmlFor="consent" className={`text-[11px] leading-relaxed cursor-pointer ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
                  I agree to the <a href="/privacy-policy" className="text-blue-500 underline hover:text-blue-400">Privacy Policy</a> and consent to Synckraft Technologies processing my data for the purpose of this inquiry.
                </label>
              </div>
